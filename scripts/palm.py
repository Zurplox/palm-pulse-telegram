...
def stories(data, limit=8):
    """Berita sawit diambil dari Palm Pulse, bukan RSS. Item tanpa judul
    dibuang, judul kembar (beda sumber) hanya diambil sekali."""
...
def stories_block(data, limit=8):
    """Blok berita sawit. "" kalau feed tidak memuat berita yang bisa dipakai,
    supaya seksinya hilang dan bukan menampilkan '(tidak ada berita)'."""
    items = stories(data, limit)
    if not items:
        return ""
    lines = []
    for item in items:
        src = (" \u2014 " + item["source"]) if item["source"] else ""
        lines.append("\u2022 %s%s" % (item["title"], src))
    return "\n".join(lines)


def palm_block(data, cfg=None, now=None):
    """Bangun blok harga TBS + ringkasan Palm Pulse. Kembalikan "" kalau tidak
    ada isi yang berguna.

    Berita Palm Pulse TIDAK ikut di sini; ada di seksinya sendiri lewat
    stories_block() supaya urutan pesan tetap rapi.
    """
    cfg = cfg or {}
    if not isinstance(data, dict):
        return ""
